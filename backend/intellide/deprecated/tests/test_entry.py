...
def entry_post(
    ...
    if file_path:
        with open(file_path, "rb") as f:
            return requests.post(url=url, headers=headers, data=data, files={"file": f}).json()
    else:
...
def entry_post_success(
    ...
    response = entry_post(user_token, entry_path, entry_type, is_collaborative, file_path)
    assert_code(response, status.HTTP_200_OK)
    return response["data"]
...
def test_entry_post_success(
    ...
    entry_post_success(user_token_default, entry_path_file, "file", "false", temp_file_path)
    ...
    entry_post_success(user_token_default, entry_path_directory, "directory", "false")
...
def test_entry_post_failure_entry_path_occupied(
    ...
    user_token_default = store["user_token_default"]
    assert_code(
        entry_post(user_token_default, entry_path_file, "file", "false", temp_file_path),
        status.HTTP_400_BAD_REQUEST,
    )
    assert_code(
        entry_post(user_token_default, entry_path_directory, "directory", "false"),
        status.HTTP_400_BAD_REQUEST,
    )
...
def test_entry_move_success_file(
    ...
    entry_post_success(user_token_default, src_entry_file_path, "file", "false", temp_file_path)
    assert_code(
        requests.put(
...
def test_entry_move_failure_entry_path_same(
    ...
    assert_code(
        requests.put(
            ...
            json={"src_entry_path": entry_path, "dst_entry_path": entry_path},
        ).json(),
        status.HTTP_400_BAD_REQUEST,
    )
...
def test_entry_move_failure_entry_path_occupied(
    ...
    entry_post_success(user_token_default, src_entry_path, "directory", "false", temp_file_path)
    entry_post_success(user_token_default, dst_entry_path, "directory", "false", temp_file_path)
    assert_code(
        requests.put(
            url=f"{SERVER_API_BASE_URL}/api/entry/move",
            headers={
            ...
            json={"src_entry_path": src_entry_path, "dst_entry_path": dst_entry_path},
        ).json(),
        status.HTTP_400_BAD_REQUEST,
    )
...
def test_entry_delete_success_file(
    ...
    entry_post_success(user_token_default, entry_path, "file", "false", temp_file_path)
...
def test_entry_delete_success_directory(
    ...
    entry_post_success(user_token_default, entry_path, "directory", "false")
    assert_code(
        requests.delete(
            url=f"{SERVER_API_BASE_URL}/api/entry",
            headers={
            ...
            params={"entry_path": path_first_n(entry_path, 3)},
        ).json(),
        status.HTTP_200_OK,
    )
    entry_paths = {d["entry_path"] for d in entry_get_success(user_token_default, "/")}
...
def test_entry_delete_failure_entry_path_not_exists(
    ...
    assert_code(
        requests.delete(
            ...
            params={"entry_path": unique_path_generator(depth=3)},
        ).json(),
        status.HTTP_400_BAD_REQUEST,
    )
