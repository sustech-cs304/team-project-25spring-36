...
from backend.config import ENCRYPT_JWT_ALGO, ENCRYPT_JWT_KEY
...
def jwt_encode(data: dict, exp_hours: Optional[int]) -> str:
    ...
    return jwt.encode(data, ENCRYPT_JWT_KEY, ENCRYPT_JWT_ALGO)
...
def jwt_verify(token: str = Header(None, alias="Access-Token")) -> dict:
    ...
    try:
        ...
        return jwt.decode(token, ENCRYPT_JWT_KEY, ENCRYPT_JWT_ALGO)
